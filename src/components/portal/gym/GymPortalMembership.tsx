...
interface Subscription {
  id: string; plan_id: string; plan_name: string | null; start_date: string; end_date: string;
  status: string; payment_status: string; amount_paid: number;
  freeze_start: string | null; freeze_end: string | null; pop_url: string | null;
}

interface GymPortalMembershipProps { memberId: string; member: GymPortalMember; }
...
export function GymPortalMembership({ memberId, member }: GymPortalMembershipProps) {
  ...
  const fetchSubscriptions = async () => {
    ...
    const mapped = ((data as any[]) || []).map((s: any) => ({
      id: s.id, plan_id: s.plan_id, plan_name: s.plan_name || s.gym_membership_plans?.name || null,
      start_date: s.start_date, end_date: s.end_date, status: s.status, payment_status: s.payment_status,
      amount_paid: Number(s.amount_paid), freeze_start: s.freeze_start, freeze_end: s.freeze_end, pop_url: s.pop_url,
    }));
    setSubscriptions(mapped);
    setLoading(false);
  };

  useEffect(() => { fetchSubscriptions(); }, [memberId]);
  ...
  const handleUpload = async (file: File, sub: Subscription) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('gym-pop').upload(path, file, { upsert: true });
      if (uploadError) throw uploadError;
      const { data: urlData } = supabase.storage.from('gym-pop').getPublicUrl(path);
      const popUrl = urlData.publicUrl + `?t=${Date.now()}`;
      const { error: updateError } = await (supabase.from('gym_member_subscriptions' as any).update({ pop_url: popUrl }).eq('id', sub.id) as any);
      if (updateError) throw updateError;
      toast.success('Receipt uploaded!');
      await fetchSubscriptions();
    } catch (err: any) { console.error(err); toast.error('Upload failed: ' + err.message); }
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-2xl bg-white/[0.04] backdrop-blur-sm border border-white/[0.06] p-5 shadow-xl">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="h-9 w-9 rounded-full bg-[#00E5A0]/10 flex items-center justify-center shrink-0">
              ...
              <div className="border-t border-white/[0.06] pt-4">
                ...
                {active.payment_status !== 'paid' && <Badge variant="destructive" className="mt-1 text-xs capitalize">{active.payment_status}</Badge>}
              </div>

              {active.status === 'frozen' && active.freeze_start && (
                <p className="mt-3 text-xs text-blue-400 flex items-center gap-1.5">
                  <Snowflake className="h-3 w-3" /> Frozen since {format(parseISO(active.freeze_start), 'dd MMM yyyy')}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              {active.pop_url ? (
                <>
                  <button onClick={() => { setProofSub(active); setProofOpen(true); }}
            ...
            <input ref={fileInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={e => { const file = e.target.files?.[0]; if (file) handleUpload(file, active); e.target.value = ''; }} />
          </div>
        );
      })() : (
        ...
        <DialogContent className="max-w-sm p-0 overflow-hidden bg-[#111118] border-white/[0.08]">
          <div className="bg-gradient-to-r from-[#00E5A0] to-[#00C4FF] p-4 text-black">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-widest text-black/40 font-semibold">Proof of Payment</p>
                <p className="font-bold text-lg leading-tight">{proofSub?.plan_name || 'Membership'}</p>
              </div>
              <button className="h-8 w-8 rounded-full bg-black/15 flex items-center justify-center" onClick={() => setProofOpen(false)}>
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div><p className="text-[10px] text-black/30 uppercase tracking-wider">Member</p><p className="font-medium">{member.first_name} {member.last_name}</p></div>
              <div><p className="text-[10px] text-black/30 uppercase tracking-wider">Member #</p><p className="font-medium">{member.member_number}</p></div>
              <div><p className="text-[10px] text-black/30 uppercase tracking-wider">Invested</p><p className="font-bold text-base">{proofSub?.amount_paid.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div>
              <div><p className="text-[10px] text-black/30 uppercase tracking-wider">Status</p><p className="font-medium capitalize">{proofSub?.payment_status}</p></div>
              {proofSub && <div className="col-span-2"><p className="text-[10px] text-black/30 uppercase tracking-wider">Period</p><p className="font-medium">{format(parseISO(proofSub.start_date), 'dd MMM yyyy')} — {format(parseISO(proofSub.end_date), 'dd MMM yyyy')}</p></div>}
            </div>
          </div>
          {proofSub?.pop_url ? (
