...
export function GymMemberPortal({ member }: GymMemberPortalProps) {
  ...
  const planTotal = data?.planEnd && data?.planName ? 30 : null; // approximate for progress
  const planProgress = daysLeft !== null && planTotal ? Math.max(((planTotal - daysLeft) / planTotal) * 100, 0) : 0;
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden animate-fade-in">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(160deg, #0a0a0a 0%, #111827 50%, #064e3b 100%)' }}
      >
        {/* Subtle glow overlay */}
        <div className="absolute inset-0 opacity-30" style={{
          background: 'radial-gradient(ellipse at 80% 20%, rgba(0,229,160,0.15) 0%, transparent 60%)'
        }} />
        ...
        {/* Today status pill */}
        {data?.todayRecord ? (
          <div className="relative flex items-center gap-2 bg-[#00E5A0]/10 border border-[#00E5A0]/20 rounded-xl px-3.5 py-2.5 w-fit">
            <span className="text-sm leading-none">✅</span>
            <span className="text-xs font-semibold text-[#00E5A0]">Checked in at {checkInTime}</span>
          </div>
        ) : (
          <div className="relative flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3.5 py-2.5 w-fit">
            <div className="h-2 w-2 rounded-full bg-white/20 shrink-0" />
            <span className="text-xs font-medium text-white/40">Not checked in yet — go to Check In</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-shrink-0 grid grid-cols-3 gap-3 px-4 py-4">
        {[
          ...
          <div
            key={label}
            className="rounded-2xl border border-white/[0.06] shadow-sm flex flex-col items-center pt-5 pb-4 px-2 gap-2"
          ...
          >
            ...
            >
              <Icon className="h-5 w-5" style={{ color, fill: color }} strokeWidth={0} />
            </div>
            <p className="text-[1.75rem] font-black text-white leading-none">{value}</p>
      ...
      <div className="flex-1 flex flex-col justify-center px-5 py-5 min-h-0">
        ...
        <blockquote className="text-[1.05rem] font-bold text-white/90 leading-snug tracking-tight mb-2 italic">
          "{quote.text}"
        </blockquote>
          ...
          <div className="px-4 py-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)' }}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <div className="h-1.5 w-1.5 rounded-full bg-[#00E5A0] shadow-[0_0_4px_#00E5A0]" />
                <span className="text-xs font-semibold text-white">{data.planName}</span>
              </div>
              <span className={cn(
                'text-xs font-bold',
                daysLeft <= 7 ? 'text-red-400' : daysLeft <= 14 ? 'text-yellow-400' : 'text-white/50'
              )}>
                {daysLeft}d left
              </span>
            </div>
