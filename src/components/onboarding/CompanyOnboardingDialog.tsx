...
import { z } from 'zod';
import { Building2, Upload, X, GraduationCap, Scale } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function CompanyOnboardingDialog({ open, onOpenChange }: CompanyOnboardingDialogProps) {
  ...
  // Check if user is a staff member — staff should NOT create company profiles
  useEffect(() => {
    if (!user?.id) return;
    const checkStaff = async () => {
      ...
      if (data) {
        ...
        onOpenChange(false); // Close dialog immediately for staff
      }
    };
    checkStaff();
  ...
  const onSubmit = async (data: OnboardingFormData) => {
    if (isStaffMember) return; // Extra guard
    setIsUploading(true);
    try {
  ...
  // Don't render anything for staff members
